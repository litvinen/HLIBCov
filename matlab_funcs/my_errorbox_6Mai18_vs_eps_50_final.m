function my_errorbox_6Mai18_vs_eps_50_final(datafile, flag)
...
eps=d2000(:,3);
ell2=d2000(:,flag);
semilogx(eps, ell2, '--', 'color', [0.0 0.6 0], 'LineWidth',2)
hold all
%plot(eps, ell2)
% .     point              -     solid
...
eps=d4000(:,3);
ell4=d4000(:,flag);
semilogx(eps, ell4, '.-', 'color', [0.1 0.5 0.1], 'LineWidth',2)
hold all

eps=d8000(:,3);
ell8=d8000(:,flag);
semilogx(eps, ell8, '-.r', 'LineWidth',2);
hold all

eps=d16000(:,3);
ell16=d16000(:,flag);
semilogx(eps, ell16, '--k', 'LineWidth',2);
hold all


eps=d32000(:,3);
ell32=d32000(:,flag);
semilogx(eps, ell32, '-m', 'LineWidth',2);
hold all

eps=d64000(:,3);
ell64=d64000(:,flag);
semilogx(eps, ell64, '.-','color', [0.5 0.5 0.1], 'LineWidth',2);
hold all
...
x=power(2, x);
epsx=(1e-8)*x;
y=truth*ones(size(epsx));
hold all
semilogx(epsx', y','r','LineWidth',3);
hold off;
legend OFF;
